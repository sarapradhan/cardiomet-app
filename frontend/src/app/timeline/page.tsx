...
export default function TimelinePage() {
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <p className="md-label" style={{ marginBottom: 4 }}>Longitudinal Tracking</p>
        <h1 className="md-headline">Your Cardiometabolic Timeline</h1>
      ...
      {/* Draw meta */}
      <div className="md-card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
          <label style={{ display: 'block' }}>
            <span style={{ display: 'block', fontSize: 13, marginBottom: 6 }}>Draw date</span>
            <input type="date" className="md-input" value={drawDate} max={new Date().toISOString().slice(0, 10)}
              onChange={(e) => setDrawDate(e.target.value)} />
          </label>
          <label style={{ display: 'block' }}>
            <span style={{ display: 'block', fontSize: 13, marginBottom: 6 }}>Label (optional)</span>
            <input type="text" className="md-input" placeholder="e.g. after starting statin"
      ...
      {/* Draws added */}
      {draws.length > 0 && (
        <div className="md-card" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <span className="md-label">Draws added ({draws.length})</span>
          {draws.map((d, i) => (
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
            <button className="md-btn-primary" onClick={analyze} disabled={isLoading}>
              {isLoading ? 'Analyzing…' : 'See My Timeline'}
            </button>
            <button className="md-btn-outlined" onClick={() => exportHealthFile({ draws })}>Export my file</button>
            <button className="md-btn-text" onClick={() => fileRef.current?.click()}>Import a file</button>
            <button className="md-btn-text" onClick={resetAll}>Clear all</button>
      ...
      {draws.length === 0 && (
        <div className="md-card" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <span className="md-body">Have a saved file?</span>
          <button className="md-btn-outlined" onClick={() => fileRef.current?.click()}>Import a file</button>
          <input ref={fileRef} type="file" accept="application/json" style={{ display: 'none' }} onChange={onImport} />
        </div>
      )}
      ...
      {error && (
        <div role="alert" style={{ padding: '12px 16px', borderRadius: 8, fontSize: 13,
          backgroundColor: 'var(--md-error-container)', color: '#410E0B' }}>{error}</div>
      )}
